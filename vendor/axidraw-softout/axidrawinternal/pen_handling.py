...
class PenHandler:
    ...
    def pen_raise(self, ad_ref):
        ...
        if self.profile_active:
            self._restore_profile_positions(ad_ref)
    ...
    def pen_raise_nonblocking(self, ad_ref):
        '''Begin raising the pen without waiting for the servo motion to finish.'''

        self.status.preview_pen_state = -1
        if self.profile_active:
            self._restore_profile_positions(ad_ref)
        if self.phys.z_up:
            return
        ...
        if not ad_ref.options.preview:
            ebb_motion.sendPenUp(ad_ref.plot_status.port, 0, servo_pin, False)
            if ad_ref.params.use_b3_out:
                ebb_motion.PBOutValue(ad_ref.plot_status.port, 3, 0, False)
        self.phys.z_up = True

    def _profile_servo_values(self, ad_ref):
        if self.heights.narrow_band:
        ...
        up_pwm = int(round(servo_min + servo_slope * ad_ref.options.pen_pos_up))
        down_pwm = int(round(servo_min + servo_slope * self.heights.pen_pos_down))
        return servo_min, servo_slope, servo_pin, up_pwm, down_pwm

    def _restore_profile_positions(self, ad_ref):
        if not self.profile_active:
            return
        _servo_min, _servo_slope, _servo_pin, up_pwm, down_pwm = \
            self._profile_servo_values(ad_ref)
        if not ad_ref.options.preview and ad_ref.plot_status.port is not None:
            ebb_motion.setPenUpPos(ad_ref.plot_status.port, up_pwm, False)
            ebb_motion.setPenDownPos(ad_ref.plot_status.port, down_pwm, False)
        self.profile_active = False

    def profile_begin(self, ad_ref):
        '''Begin a gradual path while keeping logical distance pen-down.'''
        self.status.preview_pen_state = -1
        self.profile_active = True
        self.phys.z_up = False

    def profile_height(self, ad_ref, pen_position):
        '''Set an absolute servo target without blocking the following XY move.'''
        self.status.preview_pen_state = -1
        servo_min, servo_slope, servo_pin, _up_pwm, _down_pwm = \
            self._profile_servo_values(ad_ref)
        pwm = int(round(servo_min + servo_slope * float(pen_position)))
        if not ad_ref.options.preview:
            ebb_motion.setPenUpPos(ad_ref.plot_status.port, pwm, False)
            ebb_motion.setPenDownPos(ad_ref.plot_status.port, pwm, False)
            ebb_motion.sendPenDown(ad_ref.plot_status.port, 0, servo_pin, False)
        self.profile_active = True
        self.phys.z_up = False

    def profile_finish(self, ad_ref):
        '''Restore normal servo endpoints and finish a gradual path pen-up.'''
        self.status.preview_pen_state = -1
        self._restore_profile_positions(ad_ref)
        if not ad_ref.options.preview:
            if self.heights.narrow_band:
