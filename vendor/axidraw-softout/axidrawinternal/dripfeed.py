...
def feed(ad_ref, move_list):
    ...
    for move in move_list:
        ...
        if move[0] == 'soft_raise':
            ad_ref.pen.pen_raise_nonblocking(ad_ref)
            # Keep the tail logically pen-down for distance and resume accounting.
            ad_ref.pen.phys.z_up = False
            continue

        if move[0] == 'soft_raise_finish':
            ad_ref.pen.phys.z_up = True
            continue

        if move[0] == 'profile_begin':
            ad_ref.pen.profile_begin(ad_ref)
            continue

        if move[0] == 'profile_height':
            ad_ref.pen.profile_height(ad_ref, move[1])
            continue

        if move[0] == 'profile_finish':
            ad_ref.pen.profile_finish(ad_ref)
            continue

        if move[0] == 'profile_SM':
            feed_sm(ad_ref, move, drip_logger, profile_paced=True)
            continue
...
def feed_sm(ad_ref, move, drip_logger, profile_paced=False):
    """
    ...
    if ad_ref.options.preview:
        ...
        if profile_paced and move_time > 5:
            # Gradual ramps issue a servo update between short XY segments.
            # Pace them near real time so that servo and motion commands cannot
            # saturate the EBB command queue.
            time.sleep(float(move_time - 5) / 1000.0)
        elif move_time > 50: # Sleep before issuing next command
