...
from axidrawinternal import gradual, softout
...
def trajectory(ad_ref, vertex_list, xyz_pos=None):
    ...
    start_x = xyz_pos.xpos
    start_y = xyz_pos.ypos
    ...
    if middle_moves is not None:
        middle_moves, gradual_applied = gradual.gradual_entry_exit(
            middle_moves,
            start_x,
            start_y,
            ramp_mm=getattr(ad_ref.options, "gradual_ramp_mm", 0.0),
            exit_ramp_mm=getattr(ad_ref.options, "gradual_exit_ramp_mm", None),
            tail_mm=getattr(ad_ref.options, "gradual_tail_mm", 0.0),
            segment_mm=getattr(ad_ref.options, "gradual_segment_mm", 0.5),
            pen_up=ad_ref.options.pen_pos_up,
            pen_down=ad_ref.options.pen_pos_down,
        )
        if gradual_applied:
            lift_overlapped = True
        else:
            move_list.append(['lower', None])
            middle_moves, lift_overlapped = softout.overlap_final_lift(
                middle_moves,
                start_x,
                start_y,
                getattr(ad_ref.options, "soft_out_mm", 0.0),
            )
        move_list.extend(middle_moves)
    else:
        move_list.append(['lower', None])
        lift_overlapped = False
    if not lift_overlapped:
