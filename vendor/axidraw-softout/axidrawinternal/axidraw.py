...
__version__ = '3.9.6'  # Dated 2023-12-12
...
class AxiDraw(inkex.Effect):
    ...
    def update_options(self):
        ...
        self.options.soft_out_mm = plot_utils.constrainLimits(self.options.soft_out_mm, 0, 50)
        self.options.gradual_ramp_mm = plot_utils.constrainLimits(self.options.gradual_ramp_mm, 0, 50)
        if self.options.gradual_exit_ramp_mm is not None:
            self.options.gradual_exit_ramp_mm = plot_utils.constrainLimits(
                self.options.gradual_exit_ramp_mm, 0, 50)
        self.options.gradual_tail_mm = plot_utils.constrainLimits(self.options.gradual_tail_mm, 0, 25)
        self.options.gradual_segment_mm = plot_utils.constrainLimits(
            self.options.gradual_segment_mm, 0.1, 5
        )
