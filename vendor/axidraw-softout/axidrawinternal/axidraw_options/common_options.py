...
def core_options(config):
    ...
    options.add_argument("--soft_out_mm",\
                        type=float, action="store", dest="soft_out_mm",\
                        default=float(config.get("soft_out_mm", 0.0)),\
                        help="Begin raising the pen this many mm before a path ends (0 disables)")
    options.add_argument("--gradual_ramp_mm",\
                        type=float, action="store", dest="gradual_ramp_mm",\
                        default=float(config.get("gradual_ramp_mm", 0.0)),\
                        help="Gradually lower over this path distance (0 disables)")
    options.add_argument("--gradual_exit_ramp_mm",\
                        type=float, action="store", dest="gradual_exit_ramp_mm",\
                        default=None,\
                        help="Gradually raise over this path distance (defaults to gradual_ramp_mm)")
    options.add_argument("--gradual_tail_mm",\
                        type=float, action="store", dest="gradual_tail_mm",\
                        default=float(config.get("gradual_tail_mm", 0.0)),\
                        help="Fully raised tail distance for a gradual profile")
    options.add_argument("--gradual_segment_mm",\
                        type=float, action="store", dest="gradual_segment_mm",\
                        default=float(config.get("gradual_segment_mm", 0.5)),\
                        help="XY distance between gradual pen-height targets")
